...
Despite at first glance there appearing to a large (+43%) mean increase in mileage when switching from automatic transmission to manual, taking account of the correlations of other variables lessens this impact significantly.

What remains after accounting for the other variables in the dataset is a modest 7% difference in mean gas mileage attributable to transmission type.
...
The first plot is mileage vs transmission type, the predictor and outcome we are interested in here. We see a clear difference in their means and in their overall distributions. The mileages for automatics are concentrated near its mean of $\mu_A$ = `r round(mean(mtcars[mtcars$am == 0,]$mpg), digits=1)` mpg, while those for manuals are much more even distributed along its range, with a mean of $\mu_M$ = `r round(mean(mtcars[mtcars$am == 1,]$mpg), digits=1)` mpg.
